...
from occant_utils.visualization import generate_topdown_allocentric_map,generate_topdown_allocentric_frontiermap
...
class OccAntExpTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, ans_cfg: Config) -> None:
        ...
        if not ans_cfg.LOCAL_POLICY.use_heuristic_policy:
    ...
    def save_checkpoint(
        ...
        Returns:
            None
        """
        if not self.config.RL.ANS.LOCAL_POLICY.use_heuristic_policy:
            checkpoint = {
    ...
    def resume_checkpoint(self, path=None):
        ...
        if path is None:
            if len(checkpoints) == 0:
                ...
                count_steps = 0
                count_checkpoints = 0
                if hasattr(self.config.RL.ANS.MAPPER, "INDIVISUAL_MAPPER_CKPT"):
                    # ME if use a pretrained mapper
                    print("using trained mapper")
                    ckpt_tained_mapper_dict = self.load_checkpoint(self.config.RL.ANS.MAPPER.INDIVISUAL_MAPPER_CKPT,map_location="cpu")
                    self.mapper_agent.load_state_dict(ckpt_tained_mapper_dict["mapper_state_dict"])
            else:
                ...
                if hasattr(self.config.RL.ANS.MAPPER, "INDIVISUAL_MAPPER_CKPT"):
                    # ME if use a pretrained mapper
                    ckpt_tained_mapper_dict = self.load_checkpoint(self.config.RL.ANS.MAPPER.INDIVISUAL_MAPPER_CKPT,map_location="cpu")
                    ckpt_dict["mapper_state_dict"] = ckpt_tained_mapper_dict["mapper_state_dict"]
    ...
    def _collect_rollout_step(
        ...
        for t in range(NUM_LOCAL_STEPS):
            ...
            if not self.config.RL.ANS.LOCAL_POLICY.use_heuristic_policy:  # me 210628
    ...
    def _create_global_rollouts(self, ppo_cfg, ans_cfg):
        ...
        global_observation_space = spaces.Dict(
            {
                ...
                #"frontier_mask_at_t": spaces.Box(low=0.0, high=1.0, shape=(M, M), dtype=np.float32),
                "theta_pose_at_t": spaces.Box(
                    low=0.0, high=np.pi*2, shape=(1,), dtype=np.float32),
            }
        )
        ...
        global_action_space = ActionSpace(
            {
                f"({x[0]}, {x[1]})": EmptySpace()
                for x in itertools.product(range(16), range(16))
            }
        )
        ...
        with TensorboardWriter(
            ...
            for update in range(num_updates_start, NUM_GLOBAL_UPDATES):
                for step in range(NUM_GLOBAL_STEPS):
                    logger.info("current episode:"+str(self.envs.current_episodes()[0].scene_id)+str(self.envs.current_episodes()[0].episode_id))
                    (
                        delta_pth_time,
                        delta_env_time,
                        delta_steps,
    ...
    def _eval_checkpoint(
        ...
        #me for individual mapper model,use another mapper weight; if self.config.EVAL.INDIVISUAL_MAPPER_CKPT: defined
        if hasattr(self.config.EVAL,"INDIVISUAL_MAPPER_CKPT"):
            ckpt_tained_mapper_dict = self.load_checkpoint(self.config.EVAL.INDIVISUAL_MAPPER_CKPT, map_location="cpu")
        else:
            ckpt_tained_mapper_dict={}
        ...
        if hasattr(self.config.EVAL, "INDIVISUAL_MAPPER_CKPT"):#me if use another individual mapper
            assert ckpt_tained_mapper_dict!={},"ckpt_tained_mapper_dict is NuLL."
            self.mapper_agent.load_state_dict(ckpt_tained_mapper_dict["mapper_state_dict"])
        else:
            self.mapper_agent.load_state_dict(ckpt_dict["mapper_state_dict"])
        ...
        MyAreaseen_history=defaultdict(list)#used to compute area seen
        assert self.config.T_EXP==self.config.TASK_CONFIG.ENVIRONMENT.MAX_EPISODE_STEPS, "MY T_EXP and MAX_EPISODE_STEPS Not consistent"
        for ep in range(number_of_eval_episodes):
            if ep == 0:
                observations = self.envs.reset()
            #print(observations.shape)
            current_episodes = self.envs.current_episodes()
            print("scene id and episode id:",current_episodes[0].episode_id,current_episodes[0].scene_id)
            # if ep==0:#for debug
            #     print(current_episodes)
            #     continue
            # else:
            #     if 'Mosquito' in current_episodes[0].scene_id:
            #         print('Mosquito:',current_episodes[0])
            #     else:
            #         observations = self.envs.reset()
            #         current_episodes = self.envs.current_episodes()
            ...
            for ep_step in range(self.config.T_EXP):
                ...
                writer_dict = {"writer": writer,
                               "epid": current_episodes[0].episode_id,
                               "sceneid": current_episodes[0].scene_id,
                               "ep_step": ep_step} if hasattr(ans_cfg.GLOBAL_POLICY,
                                                              "visualize_feature") and ans_cfg.GLOBAL_POLICY.visualize_feature else None
                with torch.no_grad():
                    (
                        mapper_inputs,
                        local_policy_inputs,
                ...
                observations, _, dones, infos = [list(x) for x in zip(*outputs)]
                #logger.info("observations shape:", observations.shape)
                if ep_step == 0:
                    environment_layout = np.stack(
                        [info["gt_global_map"] for info in infos], axis=0
                    )  # (bs, M, M, 2)
                ...
                if ep_step == 0 or (ep_step + 1) % 50 == 0:
                    ...
                    navigable_area_currentfloor=current_episodes[0].navigable_area_currentfloor if hasattr(current_episodes[0],"navigable_area_currentfloor") else None
                    total_area_currentfloor=current_episodes[0].total_area_currentfloor if hasattr(current_episodes[0], "total_area_currentfloor") else None
                    MyAreaseen_history[os.path.split(current_episodes[0].scene_id)[1][:-4]].append({"ep_step":ep_step,
                                                                             "area_seen":curr_area_seen_metrics["area_seen"],
                                                                            "free_space_seen":curr_area_seen_metrics["free_space_seen"],
                                                                            "occupied_space_seen":curr_area_seen_metrics["occupied_space_seen"],
                                                                            "ep_id":current_episodes[0].episode_id,
                                                                            "navigable_area_currentfloor":navigable_area_currentfloor,
                                                                            "total_area_currentfloor":total_area_currentfloor})
                    # Debug stuff
                ...
                for i in range(n_envs):
                    if (
                        ...
                        if masks[i].item() == 0:
                            ...
                            if len(self.config.VIDEO_OPTION) > 0:
                                generate_video(
                                    ...
                                    episode_id = os.path.split(current_episodes[i].scene_id)[1][:-4] + "-epid-" + str(current_episodes[i].episode_id),
                        ...
                        # episode continues
                            ...
                            H = frame.shape[0]
                            visible_occupancy_vis,_,_ = generate_topdown_allocentric_map(
                                environment_layout,
                                visible_occupancy,
                                curr_gt_poses,
                            ...
                            #visible_occupancy_vis_beforeresize = visible_occupancy_vis.copy()#for debug
                            #filename = "ep_"+str(ep_step) +"visible_occ_"+".png"
                            #cv2.imwrite("images_output/"+filename,visible_occupancy_vis_beforeresize)
                            visible_occupancy_vis = cv2.resize(visible_occupancy_vis, (H, H))
                            anticipated_occupancy_vis,_,_ = generate_topdown_allocentric_map(
                                environment_layout,
                            ...
                            #anticipated_occupancy_vis_beforeresize = anticipated_occupancy_vis.copy()#for debug
                            #filename = "ep_"+str(ep_step) +"_anticipated_occ_" +  ".png"
                            #cv2.imwrite("images_output/" + filename, anticipated_occupancy_vis_beforeresize)
                            anticipated_occupancy_vis = cv2.resize(
                                anticipated_occupancy_vis, (H, H)
                            )
                            if self.config.RL.ANS.GLOBAL_POLICY.fbe:#zoom anticipated_action_map_zoom if use fbe
                                anticipated_action_map_zoom = True#me
                                frontiermap_vis, myfrontieroffset_row, myfrontieroffset_col = generate_topdown_allocentric_frontiermap(
                                    environment_layout,
                                    visible_occupancy,
                                    curr_gt_poses,
                                    zoom=anticipated_action_map_zoom,
                                    frontier_img_draw=self.ans_net.states["frontier_img_draw"],
                                    frontier_mask=self.ans_net.states["frontier_free_mask"],
                                    thresh_explored=ans_cfg.thresh_explored,
                                    thresh_obstacle=ans_cfg.thresh_obstacle, )
                            else:
                                anticipated_action_map_zoom = False
                                frontiermap_vis=np.zeros_like(anticipated_occupancy_vis)
                                myfrontieroffset_row = 0
                                myfrontieroffset_col =0
                            anticipated_action_map,myoffset_row,myoffset_col = generate_topdown_allocentric_map(
                                environment_layout,
                                anticipated_occupancy,
                                curr_pred_poses,
                                zoom=anticipated_action_map_zoom,#me
                                thresh_explored=ans_cfg.thresh_explored,
                                thresh_obstacle=ans_cfg.thresh_obstacle,
                            )


                            #frontiermap_vis_beforeresize= frontiermap_vis.copy()#for debug
                            #filename = "ep_"+str(ep_step) +"frontiermap_" + ".png"
                            #cv2.imwrite("images_output/" + filename, frontiermap_vis_beforeresize)

                            global_goals = self.ans_net.states["curr_global_goals"]
                            local_goals = self.ans_net.states["curr_local_goals"]
                            path_way_points = self.ans_net.states["path_plan_waypoints"]

                            #draw path_way_points, path plan is performed when a new global goal is newly sampled
                            if anticipated_action_map_zoom:  # me transform coordinate
                                for tmpi in range(len(path_way_points[i][0])):
                                    tmpcol=int(path_way_points[i][0][tmpi]) - myoffset_col
                                    tmprow = int(path_way_points[i][1][tmpi]) - myoffset_row
                                ...
                                for tmpi in range(len(path_way_points[i][0])):
                                    tmpcol=int(path_way_points[i][0][tmpi])
                                    tmprow = int(path_way_points[i][1][tmpi])
                                    anticipated_action_map = cv2.circle(
                                        anticipated_action_map,
                                        (tmpcol, tmprow),
                                        3,
                            ...
                            if global_goals is not None:
                                if anticipated_action_map_zoom:#me transform coordinate
                                    cX = int(global_goals[i, 0].item())-myoffset_col
                                    cY = int(global_goals[i, 1].item())-myoffset_row
                                else:
                                ...
                                anticipated_action_map = cv2.circle(
                                    ...
                                    (255, 0, 0),
                                    -1,)
                                #frontiermap_vis = cv2.circle(frontiermap_vis,(cX, cY),6,(255, 0, 0),-1, )
                                frontiermap_vis = cv2.resize(frontiermap_vis, (H, H))

                            if local_goals is not None:
                                if anticipated_action_map_zoom:
                                    cX = int(local_goals[i, 0].item())-myoffset_col
                                    cY = int(local_goals[i, 1].item())-myoffset_row
                                else:
                            ...
                            maps_vis = np.concatenate(
                                [
                                    ...
                                    anticipated_action_map,
                                    frontiermap_vis,#np.zeros_like(anticipated_action_map),
                                ],
                                axis=1,
                            )
                            frame = cv2.resize(frame, (maps_vis.shape[1],maps_vis.shape[0]))#me:sometimes frame size doesn't match maps_vis size
                ...
                # done-if
            # done-for
            if (ep%70==0 and ep!=0) or ep==number_of_eval_episodes-1:
                #myfilename = config.TASK_CONFIG.DATASET.DATA_PATH.replace("{split}", config.TASK_CONFIG.DATASET.SPLIT)
                #myfilename, _ = os.path.split(myfilename)
                myfilename = os.path.split(config.EVAL_CKPT_PATH_DIR)[0] if os.path.isfile(config.EVAL_CKPT_PATH_DIR) else myfilename
                postfix=""
                postfix="_gibson_" if "gibson" in config.TASK_CONFIG.DATASET.DATA_PATH else postfix
                postfix="_mp3d_" if "mp3d" in config.TASK_CONFIG.DATASET.DATA_PATH else postfix
                postfix += "_trainlimit_" if "train_limit" in config.TASK_CONFIG.DATASET.DATA_PATH else postfix
                postfix+="_fbe_" if self.config.RL.ANS.GLOBAL_POLICY.fbe else postfix
                json.dump(MyAreaseen_history,open(myfilename + "/" + self.config.RL.ANS.OCCUPANCY_ANTICIPATOR.type+postfix + str(ep)+"ep_myareahistory.json","w"))
