...
def generate_topdown_allocentric_map(
    ...
    if zoom:
        ...
        return colored_map,y_start,x_start#y:row,x:col
    return colored_map,0,0

def generate_topdown_allocentric_frontiermap(
    global_map,
    pred_coverage_map,
    agent_positions,
    frontier_img_draw,
    ...
    Inputs:
        ...
        pred_coverage_map - (2, H, W) numpy array
        agent_positions   - (T, 3) numpy array --- (x, y, theta) map pose
        frontier_img_draw : frontier map with frontier info, FRONTIER_COLOR indicates all detected frontier;
                            FRONTIER_CANDIDATEPOINTS_COLOR indicates the top 3 longest condiates contour; one of them
                            will be chose randomly
        frontier_mask:      indicates the frontiers detected; 1 is for frontier 0 is for others
    """
    H, W = global_map.shape[1:]
    colored_map = np.ones((H, W, 3), np.uint8) * 255
    # global_obstacle_map = (global_map[0] == 1) & (global_map[1] == 1)
    ...
    # false_pred_obstacles = pred_obstacles & ~gt_obstacles_in_explored_map
    # colored_map[false_pred_obstacles, :] = np.array(FALSE_OBSTACLE_COLOR)
    colored_map = np.zeros((H, W, 3), np.uint8) * 255
    colored_map+=frontier_img_draw[...,[2,1,0]]# switch 0 <->2 channel
    # Draw trajectory
    if draw_trajectory:
        agent_positions_subsampled = [
            agent_positions[i] for i in range(0, len(agent_positions), 20)
    ...
    #integrate frontier information
    #colored_map = colored_map*(1-np.stack([frontier_mask,frontier_mask,frontier_mask],axis=-1))+frontier_img_draw*np.stack([frontier_mask,frontier_mask,frontier_mask],axis=-1)
