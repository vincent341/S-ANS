...
from my_util import MyFrontierAgent
...
class ActiveNeuralSLAMExplorer(ActiveNeuralSLAMBase):
    ...
    def act(
        ...
        myfrontier_agent=MyFrontierAgent.MyFrontierAgent(show_animation=False)
        ...
        if SAMPLE_GLOBAL_GOAL_FLAG:
            if self.config.GLOBAL_POLICY.fbe==False:# do not use fbe
                if self.config.GLOBAL_POLICY.combine_fbe:# me combine fbe with global policy net
                    frontier_inputmap = myfrontier_agent.ans2frontier_format(global_map)  # frontier_map [b,3,961,961] for gibson
                    _, frontier_masks_batch = myfrontier_agent.batch_sample_frontier_target(frontier_inputmap)  # frontier_masks_batch [b,M,M],ndarray,bool
                    frontier_masks_batch_tensor = torch.FloatTensor(frontier_masks_batch)

                    frontier_masks_batch_tensor = frontier_masks_batch_tensor.to("cuda")
                    global_policy_inputs = self._create_global_policy_inputs(
                        global_map, visited_states, self.states["curr_map_position"],frontier_mask=frontier_masks_batch_tensor)
                else:# me use original global policy network
                    global_policy_inputs = self._create_global_policy_inputs(
                        global_map, visited_states, self.states["curr_map_position"],cur_pose_theta=global_pose[:, 2].unsqueeze(dim=-1))
                (
                    global_value,
                    global_action,
                    global_action_log_probs,
                    _,
                ) = self.global_policy.act(global_policy_inputs, None, None, None,deterministic=self.config.GLOBAL_POLICY.deterministic,writer_dict=writer_dict)# returned global_action is in 16*16 map
                #me: delete del observations['frontier_mask'],or will cause error later
                # if 'frontier_mask' in global_policy_inputs.keys():
                #     #del global_policy_inputs['frontier_mask']
                #     global_policy_inputs.pop('frontier_mask',None)
                # Convert action to location (row-major format)
                #first convert 16*16 action to G*G local map coordinate==========================================================
                tmp_mod=torch.fmod(global_action, self.global_policy.actor_critic.actor_outsize).float() # ind%16
                tmp_div = (global_action/ self.global_policy.actor_critic.actor_outsize).float() #ind/16
                tmp_scale_factor = self.global_policy.G//self.global_policy.actor_critic.actor_outsize
                global_action_inGGmap = tmp_scale_factor*tmp_mod+tmp_scale_factor*tmp_div*self.global_policy.G#now global_action is under coordinate of local map G*G. localmap? not sure
                #====================================================================================================
                G = self.global_policy.G
                if self.global_policy.config.myego_localmap and self.global_policy.config.myuse_localmap and (not self.global_policy.config.myuse_globalmap):
                    # only ego_local map; rotate and translate
                    #convert egocentric cor in local map to allocentric cor in global map; after this, cor is in global pixel map
                    tmp_mod = torch.fmod(global_action_inGGmap, self.global_policy.G).float()  # col
                    tmp_div = (global_action_inGGmap / self.global_policy.G).float()  # row
                    global_action_map_x,global_action_map_y=self.global_policy.localcor2globalcor(local_cor=torch.cat((tmp_mod,tmp_div),dim=1),
                                                          theta=global_pose[:, 2].unsqueeze(dim=-1),
                                                          agent_pos=self.states["curr_map_position"],
                                                            G=self.global_policy.G,translate=True)
                elif self.global_policy.config.myuse_localmap and (not self.global_policy.config.myuse_globalmap) and (not self.global_policy.config.myego_localmap):
                    #case use allo local map only. Translate
                    tmp_mod = torch.fmod(global_action_inGGmap, self.global_policy.G).float()  # col
                    tmp_div = (global_action_inGGmap / self.global_policy.G).float()  # row
                    global_action_map_x, global_action_map_y = self.global_policy.localcor2globalcor(
                        local_cor=torch.cat((tmp_mod, tmp_div), dim=1),
                        theta=torch.zeros_like(global_pose[:, 2].unsqueeze(dim=-1)).to(global_pose.device),#rotate 0
                        agent_pos=self.states["curr_map_position"],
                        G=self.global_policy.G, translate=True)
                elif self.global_policy.config.myuse_globalmap and not (self.global_policy.config.myego_globalmap):
                    #case: allocentric global map & case: allocentric cat global and local map
                    #convert cor in allocentric local map to allocentric global map
                    global_action_map_x = torch.fmod(
                        global_action_inGGmap.squeeze(1), G
                    ).float()  # (bs, )
                    global_action_map_y = (global_action_inGGmap.squeeze(1) / G).float()  # (bs, )
                    global_action_map_x = global_action_map_x * M / G  # me col
                    global_action_map_y = global_action_map_y * M / G  # me row
                elif self.global_policy.config.myuse_globalmap and self.global_policy.config.myego_globalmap:
                    #case ego_global map only & case: ego cat global local
                    tmp_mod = torch.fmod(global_action_inGGmap, self.global_policy.G).float()  # col
                    tmp_div = (global_action_inGGmap / self.global_policy.G).float()  # row
                    global_action_map_x, global_action_map_y = self.global_policy.localcor2globalcor(
                        local_cor=torch.cat((tmp_mod, tmp_div), dim=1),
                        theta=global_pose[:, 2].unsqueeze(dim=-1),
                        agent_pos=self.states["curr_map_position"],
                        G=self.global_policy.G, translate=False)#only rotate
                    global_action_map_x = global_action_map_x * M / G  # me col
                    global_action_map_y = global_action_map_y * M / G  # me row
                else:
                    raise ValueError("Unexpected cor transformation!")
                global_action_map_x = torch.clamp((global_action_map_x.ceil()), min=0,
                                                  max=global_map.shape[-1] - 1)  # cor in M*M map cor system
                global_action_map_y = torch.clamp((global_action_map_y.ceil()), min=0, max=global_map.shape[-1] - 1)
                # G = self.global_policy.G
                # global_action_map_x = torch.fmod(
                ...
                # if self.config.GLOBAL_POLICY.combine_fbe:# use crop map G*G 's coordinate
                #     global_action_map_x=global_action_map_x+global_policy_inputs["pose_in_map_at_t"][:,0]-G//2
                #     global_action_map_y = global_action_map_y + global_policy_inputs["pose_in_map_at_t"][:,1] - G // 2
                # else:
                #     #me the following use resized map , rather than crop map
                #     global_action_map_x = global_action_map_x * M / G#me col
                #     global_action_map_y = global_action_map_y * M / G#me row
                global_action_map_xy = torch.stack([global_action_map_x, global_action_map_y], dim=1)
                # Set the goal (bs, 2) --- (x, y) in map image coordinates
                self.states["curr_global_goals"] = global_action_map_xy
                #print("global_action_map_xy[0]:",global_action_map_xy[0].cpu().data.numpy())
            else:#use fbe
                frontier_inputmap=myfrontier_agent.ans2frontier_format(global_map)#frontier_map [b,3,961,961] for gibson
                frontier_targetpoints,_=myfrontier_agent.batch_sample_frontier_target(frontier_inputmap)#frontier_targetpoints [b,2],numpy
                frontier_targetpoints_tensor = torch.FloatTensor(frontier_targetpoints)
                frontier_targetpoints_tensor = frontier_targetpoints_tensor.to("cuda")
                self.states["frontier_img_draw"]=myfrontier_agent.frontier_img_draw
                self.states["frontier_free_mask"] = myfrontier_agent.free_mask#(255,255,255) is frontier
                 # Set the goal (bs, 2) --- (x, y) in map image coordinates
                #self.states["curr_global_goals"] = global_action_map_xy
                self.states["curr_global_goals"] = frontier_targetpoints_tensor#x:col y:row
    ...
    def train(self):
        ...
        self.local_policy.train()
        self.global_policy.actor_critic.train()

    def eval(self):
        self.mapper.eval()
        self.global_policy.eval()
        self.local_policy.eval()
        self.global_policy.actor_critic.eval()
    ...
    def _compute_plans_and_local_goals(
        ...
        if SAMPLE_LOCAL_GOAL_FLAGS:# me tested for evaluation mode
            #me, for drawing path points;SAMPLE_LOCAL_GOAL_FLAGS is true when global goal is newly sampled
            #plans_xy is none if SAMPLE_LOCAL_GOAL_FLAGS==False
            if (None,None) not in plans_xy:
                self.states["path_plan_waypoints"] = plans_xy
        # ========================= Sample a local goal =======================
        ...
        for i in range(self.nplanners):
            ...
            if path_x is None:#global goal is not sampled
                ...
                #print("path_x is None in _compute_plans_and_local_goals ",self.config.recovery_heuristic,"\n")
    ...
    def _create_global_policy_inputs(self, global_map, visited_states, map_xy,cur_pose_theta=None,frontier_mask=None):
        """
        ...
        cur_pose_theta, me ,theta in pose (x,y,theta)
        frontier_mask: me [bs,M,M], tensor,float,0.0 or 1.0; frotiner area 1.0, else 0.0
        """
        ...
        h_t = torch.cat([global_map, visited_states, agent_position_onehot], dim=1)  # (bs, 4, M, M)

        global_policy_inputs = {
            "pose_in_map_at_t": map_xy,
            "map_at_t": h_t,
            #"frontier_mask_at_t":frontier_mask,
            "theta_pose_at_t":cur_pose_theta #[b,1]
        }
        #print("cur_pose_theta shape:",global_policy_inputs["theta_pose_at_t"].shape)
...
class ActiveNeuralSLAMNavigator(ActiveNeuralSLAMBase):
    ...
    def act(
        ...
        if ep_step == 0:
            ...
            print("ep_step:",ep_step,"-curr_dist2localgoal:",self.states["curr_dist2localgoal"],"The previous local goal was reached","\n")
